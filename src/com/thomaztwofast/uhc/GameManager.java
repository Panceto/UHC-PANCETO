...
public class GameManager {
	...
	public static int cuentaatras = 14400;
	...
	public void startGame(int value) {
		...
		if (pl.config.gameInTeam) {
			...
			if (scoreboard.getObjective("UHC SANCHETO") == null)
				//scoreboard.getObjective("UHC SANCHETO").unregister();
				scoreboard.registerNewObjective("UHC SANCHETO", "V1","UHC SANCHETO").setDisplaySlot(DisplaySlot.SIDEBAR);
				Bukkit.getScheduler().runTaskTimer(pl, new Runnable() {
				@Override
				public void run() {
					cuentaatras--;
					int horas = (cuentaatras / 60) / 60;
				 	int minutos = ((cuentaatras / 60) % 60);
					int segundos = (cuentaatras % 60 );
					String result = horas + ":" + minutos + ":" + segundos;
					scoreboard.getObjective("UHC SANCHETO").setDisplayName(
					ChatColor.GOLD.toString() + ChatColor.BOLD + "TIME:" + ChatColor.WHITE + result);

					Score s = scoreboard.getObjective("UHC SANCHETO").getScore("");
					//Score s2 = scoreboard.getObjective("UHC SANCHETO").getScore("minutos: ");
					//Score s3 = scoreboard.getObjective("UHC SANCHETO").getScore("segundos: ");
					//s.setScore(horas);
					//s2.setScore(minutos);
					//s3.setScore(segundos);
					s.setScore(1);
					if(cuentaatras < 0) {
		...
		str.addText("\n \u00A79> \u00A7ATeletransporte en progreso");
		str.addText("\u00A79> \u00A76El juego empezara en  \u00A7C" + (countdownData[1] + (countdownData[0] / 20)) + "\u00A76 segundos.");
		str.addText("\u00A76\u00A7LSuerte, y cuidado con los creepers");
	...
	private void endGame() {
		pl.getServer().getScheduler().runTaskLater(pl, new Runnable() {
			public void run() {
				...
				S str = new S(false);
				str.setTitle("UHC Sancheto");
				str.addText("\n " + winner + "\u00A76\u00A7l Ha ganado el juego.");
				pl.PLAYERS.values().forEach(e -> {
					e.player.sendMessage(str.print());
					e.player.sendTitle(winner, "\u00A77Gana el juego", 10, 60, 10);
	...
	private void setPlayerAsSpectator(UHCPlayer u) {
		u.resetPlayer(false);
		u.sendCmdMessage("UHC Sancheto", "Ahora estas en modo espectador");
		if (pl.config.serverEnable) {
				...
				inGamePlayers.forEach(e -> {
					if (pl.getRegisterPlayer(e) != null) {
						...
						if (countdownData[1] == 0) {
							...
							u.player.sendTitle("", "\u00A76Comenzando en \u00A7C\u00A7L" + countdownData[1] + "\u00A7R\u00A76 Segundos" + (countdownData[1] == 1 ? "" : "s"), 5, 21, 5);
	...
	private void taskMarkers() {
		gameTask[1] = pl.getServer().getScheduler().runTaskTimer(pl, new Runnable() {
			public void run() {
				...
				if(cuentaatras > 1800 ) {
					broadcastMessage("La arena se hace mas pequeña");	
				}
				if(cuentaatras < 1800 && cuentaatras > 0){
					broadcastMessage("dirigiros a las coordenadas (0,0) y cuidado fuera del area perderas vida por segundo");
				}
