	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		if (sender instanceof Player) {
			...
			if (pl.config.pluginEnable && pl.status.ordinal() == 4) {
					...
					case -1166190107:
						if (pl.config.gameInTeam && args.length == 2) {
							if (pl.PLAYERS.containsKey(args[1]) && u.player.getScoreboard().getEntryTeam(args[1]) == null) {
								...
								t.sendCmdMessage("UHC Sancheto", "El juego va a empezar selecciona un equipo o te asignaran el modo espectador.");
				...
				J str = new J("                                                                   \n", "8", "s");
				str.addText(" UHC Sancheto - " + (pl.config.gameInTeam ? "TEAM" : "SOLO") + " MODE\n\n", "f", "b");
				...
				if (pl.config.gameInTeam) {
					if (entries.size() > 1) {
						...
						str.addText("\n\n Se te asigna el modo espectador.\n", "8", "i");
					}
				}
				if (Integer.valueOf(entries.get(0)) < 2) {
					...
					str.addText(" Ejecuta el comando de inicio\n", "7", "");
